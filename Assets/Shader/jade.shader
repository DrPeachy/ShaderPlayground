...
////// Emissive:
                ...
                float3 emissive = saturate((1.0-(1.0-(pow(1.0-max(0,dot(normalDirection, viewDirection)),_fresnel_var)*float4(1,1,1,1)))*(1.0-lerp((_Color_var.rgb*(_rampTex_var.rgb*_darkness_factor_var)),_specular_color_var.rgb,max(step(_specular_rang_1_var,(_remap_var.b + ( (dot(i.normalDir,normalize((_specular_offset_var.rgb+lightDirection))) - _remap_var.r) * (_remap_var.a - _remap_var.b) ) / (_remap_var.g - _remap_var.r))),step(_specular_range_2_var,node_5944)))))).rgb;
